...
let i;
for (i = 0; i <= 10; i++) {
  calcularMultiplicador(i)
  console.log('')
}

function calcularMultiplicador(n) {
  let i;
  for (i = 0; i <= 10; i++) {
    let multiplicación = `${n} X ${i} = ${i * n}`
...
function calcularGradosCelsiusFahrenheit(n) {
  let fahrenheit = n * 9 / 5 + 32
  console.log(fahrenheit)
}

calcularGradosCelsiusFahrenheit(prompt(`ingresar grados`))
...
function calcularGradosFahrenheitCelsius(n){
  let celcius = (n - 32) * 5/9
  console.log(celcius)
}

calcularGradosFahrenheitCelsius(prompt(`INGRESAR GRADOS`))
...
let i;
let numerosArray = [1, 1, 1, 1, 1, 1, 1]
let totalSuma = 0;

for (i = 0; i < numerosArray.length; i++) {
  totalSuma = numerosArray[i] + totalSuma;
}
...
function promedioDeArray() {
  let i;
  let numerosArray = [4, 6, 9, 3, 8, 7, 5];
  let sumaArray = 0;
  let promedioArray = 0;
  for (i = 0; i < numerosArray.length; i++) {
    sumaArray = numerosArray[i] + sumaArray
  }
  promedioArray = sumaArray / numerosArray.length
  console.log(`tu promedio es de ${promedioArray}`)
}
