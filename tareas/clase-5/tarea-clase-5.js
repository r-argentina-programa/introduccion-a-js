...
document.querySelector('#calcularSalarioMensual').onclick = function () {
  const salarioAnualUsuario = Number(document.querySelector('#salarioAnual').value);
  const salarioMensualUsuario = calculoDelMes(salarioAnualUsuario);

  document.querySelector('#salarioMensual').value = salarioMensualUsuario;

  return false;
}

function calculoDelMes(salarioAnualUsuario) {
  return Math.round(salarioAnualUsuario/12);
}
