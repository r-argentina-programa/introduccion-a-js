...
let comidasFavoritas = ["milanesas", "kippe", "empanadas"];
...
comidasFavoritas[0] = "Milanesas de Pollo";
...
/* const notasParcialAlumno = [5, 7, 4, 9, 10];

function calculadorPromedioNotas(arrayNotasParcialAlumno) {
	let sumaNotas = 0;
	let promedio;
	for (i = 0; i < arrayNotasParcialAlumno.length; i++) {
		sumaNotas += arrayNotasParcialAlumno[i];
	}
	promedio = sumaNotas / arrayNotasParcialAlumno.length;
	return promedio;
}

console.log(`Tu promedio es ${calculadorPromedioNotas(notasParcialAlumno)}`); */
